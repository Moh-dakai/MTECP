...
<head>
    ...
    <title>{{ tenant('name') ?? 'Storefront' }}</title>
    ...
    <style>
        :root {
            --color-primary: {{ tenant('color_primary') ?? '#4f46e5' }};
            --color-secondary: {{ tenant('color_secondary') ?? '#4338ca' }};
        }
    ...
    <div class="min-h-screen flex flex-col">
                ...
                <div>
                    @if (Route::has('login'))
                        <div class="flex space-x-4 items-center">
                            ...
                            @auth
                                <a href="{{ url('/dashboard') }}" class="text-sm font-semibold text-gray-600 hover:text-brand-primary">Dashboard</a>
                            @else
                                <a href="{{ route('login') }}" class="text-sm font-semibold text-gray-600 hover:text-brand-primary">Log in</a>
                            @endauth
                        </div>
                    @endif
                </div>
        ...
        <footer class="bg-gray-800 text-white mt-12 py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8">
                <div>
                    <h3 class="text-lg font-bold mb-4">{{ tenant('name') ?? 'Storefront' }}</h3>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 pt-8 border-t border-gray-700 text-sm text-center text-gray-400">
                &copy; {{ date('Y') }} {{ tenant('name') ?? 'Storefront' }}. All rights reserved.
            </div>
        </footer>
    </div>
    @livewireScripts
